...
exports.resubmitApplication = async (req, res) => {
  try {
    ...
    if (now < rejectedAt + TWO_HOURS) {
      const hoursLeft = Math.ceil((rejectedAt + TWO_HOURS - now) / (60 * 60 * 1000));
      return res.status(400).json({ 
    ...
    // Delete old 
    await HostApplication.findOneAndDelete({ user: userId });
    await User.findByIdAndUpdate(userId, { hostApplicationStatus: 'none' });

    res.json({ message: 'You can now submit a fresh application', canSubmitFresh: true });
  } catch (error) {
    console.error('Resubmit application error:', error);
    res.status(500).json({ message: 'Failed to process resubmit' });
  }
};
