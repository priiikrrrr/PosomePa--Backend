  ...
  try {
    ...
    contentBased.forEach((space, index) => {
      spaceMap.set(space._id.toString(), {
        space,
        score: (contentBased.length - index) * CONTENT_WEIGHT
      });
    });
    ...
    collaborative.forEach((space, index) => {
      ...
      if (existing) {
        existing.score += (collaborative.length - index) * COLLABORATIVE_WEIGHT;
      } else {
        spaceMap.set(space._id.toString(), {
          space,
          score: (collaborative.length - index) * COLLABORATIVE_WEIGHT
        });
      }
    });
